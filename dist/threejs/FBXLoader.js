...
(function () {
	...
	THREE.FBXLoader.prototype.addAnimation = function (mesh, matrices, animations) {
		...
		var hasCurve = function hasCurve(animNode, attr) {
		...
		var hasKeyOnFrame = function hasKeyOnFrame(attrNode, frame) {
		...
		var isKeyExistOnFrame = function isKeyExistOnFrame(curve, frame) {

			var value = curve.values[frame];
			return value !== undefined;
		};

		var genKey = function genKey(animNode, bone) {
   ...
   */
	...
	Weights.prototype.parse = function (node, bones) {
		...
		for (var i = 0; i < weights.length; i++) {
			...
			var weight = new THREE.Vector4(weights[i].weight[0] ? weights[i].weight[0] : 0, weights[i].weight[1] ? weights[i].weight[1] : 0, weights[i].weight[2] ? weights[i].weight[2] : 0, weights[i].weight[3] ? weights[i].weight[3] : 0);
  ...
  */
	...
	parse_Data_ByPolygonVertex_Direct = function parse_Data_ByPolygonVertex_Direct(node, indices, strides, itemSize) {
	...
	var FBXTimeToSeconds = function FBXTimeToSeconds(adskTime) {
	...
	quatFromVec = function quatFromVec(x, y, z) {
	...
	toMat44 = function toMat44(arr) {

		var mat = new THREE.Matrix4();
		mat.set(arr[0], arr[4], arr[8], arr[12], arr[1], arr[5], arr[9], arr[13], arr[2], arr[6], arr[10], arr[14], arr[3], arr[7], arr[11], arr[15]);
